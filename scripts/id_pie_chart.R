"Usage: id_part1.R (--out1 <O1>) <input1> <input2> <input3> <input4> <input5> <input6> <input7> <input8> <input9>
-h --help    show this
...
opts <- docopt(doc)
save.image("id_pie_chart.rda")

# Gráfico da distribuição de marcas no genoma
...
# Lê o conjunto de cada tecido

for(l in c(opts$`<input8>`, opts$`<input9>`)){
    
    group_name <- strsplit(as.character(l), "sites_")[[1]][2]
    group_name <- strsplit(as.character(group_name), ".tst")[[1]]
    
    metil <- read.table(paste(l), colClasses = "character", header = T)
    ...
    for (i in 1:length(metil)){
        ...
        #    exon_overlaping_genes <- unique(exon_overlaping_genes)
        ...
        print(paste0("Genomic contexct of ",tissue, " group ", group_name, ":"))
        print(dat_sub)
        ...
        svg(paste(paste0("images/genomic_context/", group_name, "_", tissue), "Mehylated_sites_distribution.svg", sep = "_"),
            width = 8,
            height = 8,
            pointsize = 12)
...
#     metil <- read.table(paste(l), colClasses = "character", header = T)
#     
#     tissue_methylation <- read.table(paste(l),
...
#     inter_12_full <- unique(intersect(sample1, sample2)) # interseção12
#     inter_13_full <- unique(intersect(sample1, sample3)) # interseção13
#     inter_23_full <- unique(intersect(sample2, sample3)) # interseção23
#     
#     # tamanho para o plot
#     inter_123_full <- unique(intersect(inter_12_full, sample3)) # interseção123
#     
...
#     # write a table with the counts in each category
#     write.table(dat_sub,
#                 paste("intersection", group_name, "Mehylated_sites_distribution_graph_table.tst", sep = "_"),
#                 col.names = T,
#                 row.names = F,
#                 sep = "\t",
#                 quote = F)
...
#                    show.legend = F)
#     
#     svg(paste(paste0("images/genomic_context/", "intersection"), group_name, "Mehylated_sites_distribution.svg", sep = "_"),
#         width = 8,
#         height = 8,
#         pointsize = 12)
#     
...
###############################################################
### Comparing the subsets of DNA methylation among clones   ###
###############################################################
...
#     tissue_methylation <- read.table(l, colClasses = "character", header = T)
#     
...
#         
#         # print(paste(dfnames[i]))
#         # print(paste("AL", nrow(AL)))
#         # print(paste("JL", nrow(JL)))
#         # print(paste("XY", nrow(XY)))
#         # print(paste("all", nrow(dfList[[i]])))   
#         
