...
# Extraí somente os sítios na fita plus
sites <- sites_bed[sites_bed[, 6] == "+", ]
names(sites) <- c("chr", "start", "end", "name", "score", "strand")

# Executa a busca pelos sítios mais próximos de cada marca
# Defines the function to be used to the restriction sites search
closest_sites_search <- function(i, marks_pos, sites){
    ...
    # Extrái as informações da marca avaliada
    ...
    # Extraí os sítios de restrição apenas para o chr ou scaffold da marca.
    sites_feature <- sites[sites[, 1] == feature, ]
    
    if (strand_target == "+"){
        
        ## Extraí o sítio downstream ao fragmento
        ...
        ## Organiza as colunas para o formato bed
        ...
        ## Junta os sítios em um único dataframe
        ...
        ## Extraí o sítio upstream ao fragmento
...
# Remove as marcas para as quais não é possível determinar as extremidades do fragmento.
...
# Remove as marcas que não tem sítio PstI-MspI da análise.
...
# Escreve o arquivo bed - contém diversas colunas acessórias
...
# Remove os sítios não-determinados da planilha de contagem.
