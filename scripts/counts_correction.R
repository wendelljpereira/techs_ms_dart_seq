...
## Normaliza as contagens usando as tags como referência.
...
for (i in unique(temp_bed$V7)){
    ...
    #     
        ...
        print(paste("Warning: More than 3 features in cluster ", i, "!", sep = ""))
    }
}

# Exclui as contagens e features que foram normalizadas.
all_counts <- all_counts[!all_counts$Geneid %in% temp_counts$Geneid, ]

# Adiciona as contagens normalizadas, ordena e cria um novo nome.
...
# Mantem apenas as colunas com as contagens
all_counts <- all_counts[,-c(2,3,4,5,6)]

# Esclui as features que foram modificadas, adiciona as novas, ordena e renomeia de forma compativel com as contagens
...
# Salva os novos arquivos já ordenados
