...
#foreach(i=1:length(files), .inorder = FALSE, .combine=c) %dopar% {
for(i in 1:length(files)){
    ...
    # Testa se existe um sítio pstI
    if("PstI" %in% unique(cluster_with_overlaps$V4)){

        # testa em qual strand está o cluster
        if(unique(cluster_with_overlaps$V6) == "+"){
            ...
            #    new_representative_clusters <- rbind(new_representative_clusters, new_positions_frags)
