...
# Function to descritive statistics.
...
# Select only the end position for each restriction site
frags_genome_plus <- frags_genome_plus[, c(1, 3, 4, 6)]

# Help function to use in apply - change the end position according with the cut from enzyme
...
# Set the objects which will recive the results of fragments_determination()
...
save(file = "all_frags.rda", all_frags_df) # <<<<<<<<<<<<<<<<
...
# Using the results of plus strand and, considering the cuts patterns of each enzime, is possible to determines the fragments generated in minus strand without execution the analysis again.

# Fita minus - PstI-PstI frags
...
# rbind all fragments of the minus strand in a data.frame
...
# rbind plus and minus fragments
all_frags_genome <- rbind(frags_genome_plus, frags_genome_minus)

# Generate the descritive statistics for the set of fragments generated by
#the double (PstI-MspI) digestion.
...
# Generate the descritive statistics for the size of fragments selected by DArTseq.
...
# Generate the descriptive statistics for the size of fragments selected by DArTseq.
PstI_MspI <- all_frags_genome[grep("PstI-MspI", all_frags_genome$type), ]
#MspI_PstI <- all_frags_genome[grep("MspI-PstI", all_frags_genome$type), ]
frags_msp_pst <- rbind(PstI_MspI)

# Generate the descriptive statistics for the size of fragments avaliable to sequecing by DArTseq.
...
possible_to_select_frags <- data.frame(sample = rep("PstI-MspI fragments", nrow(frags_msp_pst)), fragments_size = frags_msp_pst$fragments_size)

selected_frags <- data.frame(sample = rep("Sequenced fragments", length(sampled_frag_sizes)), fragments_size = sampled_frag_sizes)
